...
export default function Home() {
  ...
  const loadProjectData = async (file: string, projectName: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: `✅ **${projectName}**\n\n📊 Key Metrics ('000):\n• BP GP: $${data.metrics['Business Plan GP'].toLocaleString()}\n• Proj GP: $${data.metrics['Projected GP'].toLocaleString()}\n• WIP GP: $${data.metrics['WIP GP'].toLocaleString()}\n• Cash Flow: $${data.metrics['Cash Flow'].toLocaleString()}\n\n📁 **Source:** ${data.debug?.source || 'N/A'}\n📋 **Rows:** ${data.debug?.totalRows || 0}\n\n**Sample Parsed Rows:**\n${data.debug?.sampleRows?.map((r: any, i: number) => `${i+1}. Sheet="${r.Sheet}" FinType="${r.FinType}" Item="${r.Item}" Value=${r.Value}`).join('\n') || 'N/A'}\n\n📑 **Unique Sheets:** ${data.debug?.uniqueSheets?.join(', ') || 'N/A'}\n🏷️ **Unique FinTypes:** ${data.debug?.uniqueFinancialTypes?.join(', ') || 'N/A'}\n🔢 **Unique ItemCodes:** ${data.debug?.uniqueItemCodes?.join(', ') || 'N/A'}\n🏷️ **Unique DataTypes:** ${data.debug?.uniqueDataTypes?.join(', ') || 'N/A'}\n🔍 **GP Rows (Item=3, Data~gro...):** ${data.debug?.gpRowsCount || 0}`
      }])
    } catch (error) {
